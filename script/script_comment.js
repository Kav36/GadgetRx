...
(function() {
    ...
    .then((data) => {
      ...
      loadRating(data.username, shopIdInput.value);
      fetch(`/get_profilePic?username=${data.username}`)
      .then(response => response.json())
      .then(data => {
        if (data.length > 0 && data[0].profilePic) {
          document.getElementById('person_pic').value = data[0].profilePic;
        } else {
        }
      })
      .catch(error => {
    ...
    function appendComment(data) {
        ...
        var newCommentHtml = commentTemplate.innerHTML
            ...
            .replace('{{profilePic}}', data.profilePic || document.getElementById('person_pic').value) // Use a default image if profilePic is not provided
