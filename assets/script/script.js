        ...
        card.innerHTML = `
                      <div class="card">
                          ...
                          <div class="card-body">
                              ...
                              <span class="badge text-bg-light mb-3">${tech.tech_qul}</span>
...
function searchShops() {
  ...
  fetch(url)
    ...
    .then((shops) => {
      ...
      if (shops.length === 0) {
        container.innerText =
          "No shops found! Please try again with different search criteria.";
        return;
      }
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <a href="#" class="btn btn-primary" onclick="showShopDetails('${
          shop.shop_name
        }')">Details</a>
      </div>
    </div>
...
function setShopdata(shopId) {
  fetch(`http://localhost:3000/setShopdata?shop_id=${shopId}`)
    ...
    .then((data) => {
      ...
      fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${data.latitude},${data.longitude}&key=AIzaSyCDx78wjiGh46Ei1oL-9b8M1-5zogF1VXs`)
        .then((response) => response.json())
        .then((geoData) => {
          if (geoData.error_message || geoData.status !== "OK") {
            console.error("Error fetching address:", geoData.error_message || geoData.status);
            return;
          }
